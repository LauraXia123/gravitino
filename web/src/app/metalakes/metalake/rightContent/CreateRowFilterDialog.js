...
import { createRowFilter, updateRowFilter } from '@/lib/store/metalakes'
...
const CreateRowFilterDialog = props => {
  ...
  const onSubmit = data => {
    ...
    schema
      ...
      .then(() => {
        if (type === 'create') {
          dispatch(createRowFilter({ data, metalake })).then(res => {
            if (!res.payload?.err) {
              handleClose()
            }
          })
          ...
          if (reqData.updates.length !== 0) {
            dispatch(updateRowFilter({ metalake, tag: cacheData.name, data: reqData })).then(res => {
              if (!res.payload?.err) {
                handleClose()
              }
            })
        ...
        >
          ...
          <Box sx={{ mb: 8, textAlign: 'center' }}>
            <Typography variant='h5' sx={{ mb: 3 }}>
              {type === 'create' ? 'Create' : 'Edit'} Row Filter
            </Typography>
          </Box>
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <TextField
                      ...
                      data-refer='row-filter-name-field'
                    />
                  )}
                />
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <TextField
                      ...
                      data-refer='row-filter-expression-field'
                    />
                  )}
                />
                {errors.expression && (
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange } }) => (
                    <TextField
                      ...
                      data-refer='row-filter-description-field'
                    />
                  )}
                />
              </FormControl>
...
export default CreateRowFilterDialog
